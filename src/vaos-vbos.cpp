...
void DescrVBOAtribs::crearVBO() 
{
   ...
   glGenBuffers(1, &buffer);  // 'buffer' es un miembro de la clase, donde se almacenará el id del VBO
...
void DescrVBOInds::crearVBO( )
{
   ...
   glGenBuffers(1, &buffer);  // 'buffer' es un miembro de la clase donde se almacenará el identificador del VBO
...
void DescrVAO::crearVAO()
{
   ...
   glGenVertexArrays(1, &array);  // Generar el identificador del VAO
   assert(0 < array);
   glBindVertexArray(array);      // Activar el VAO, lo convierte en el VAO actual
   ...
   for (GLuint i = 0; i < num_atribs; i++) {
      if (dvbo_atributo[i] != nullptr) {
         dvbo_atributo[i]->crearVBO();  // Crear el VBO de atributos en la GPU
      }
   }
   ...
   if (dvbo_indices != nullptr) {
      dvbo_indices->crearVBO();  // Crear el VBO de índices en la GPU
   }
   ...
   for (unsigned int i = 0; i < num_atribs; i++) {
      if (dvbo_atributo[i] != nullptr && !atrib_habilitado[i]) {   
         glDisableVertexAttribArray(dvbo_atributo.at(i)->buffer);  // Deshabilita el atributo en el índice
      }
   }
...
void DescrVAO::draw( const GLenum mode )
{
   ...
   // 
   if (array == 0) {
      // Si el VAO no ha sido creado aún (array == 0), crearlo
      crearVAO();
   } else {
      // Si el VAO ya ha sido creado, activarlo
      glBindVertexArray(array);
   }
