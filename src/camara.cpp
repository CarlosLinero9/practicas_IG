...
void Camara3Modos::desplRotarXY( const float da, const float db )
{
   switch( modo_actual )
   {
      case ModoCam::examinar :
      {
         ...
         // .....
         org_polares[0] += da/100.0;
         org_polares[1] += db/100.0;
      ...
      case ModoCam::prim_pers_rot :
      {
         ...
         org_polares[0] += da/100.0;
         org_polares[1] += db/100.0;
         vec3 anterior = org_cartesianas;
         org_cartesianas = Cartesianas(org_polares);
         punto_atencion=punto_atencion+(anterior - org_cartesianas);
...
void Camara3Modos::moverZ( const float dz )
{
   ...
   switch( modo_actual )
   {
      case ModoCam::examinar :
      {
         ...
         // .....
         float ajustar = 0.5f;
         org_polares[Z] = ajustar + (org_polares[Z] - ajustar)*pow(1.1, dz);
         org_cartesianas = Cartesianas(org_polares);
         break ;
      }
      case ModoCam::prim_pers_rot :
      case ModoCam::prim_pers_despl :
      {
         ...
         // nota: los ejes no cambian
         // .....
         vec3 variar = dz*eje[Z];
         punto_atencion = punto_atencion + variar;
         org_cartesianas = org_cartesianas + variar;
         org_polares = Esfericas(org_cartesianas);
         break ;
      }
   }
